...
describe("lexer", () => {
  ...
  it("treats non-keyword identifiers (like yap) as IDENTIFIER", () => {
    const toks = tokenize("yap howmany squad");
  ...
  it("ignores // comments to end of line", () => {
    expect(types("manifest x = 5 // this is a comment\nx")).toEqual([
      TokenType.VAR,
      TokenType.IDENTIFIER,
      TokenType.EQUAL,
      TokenType.NUMBER,
  ...
  it("tracks line and column numbers", () => {
    const toks = tokenize("manifest x\n  y");
    expect(toks[0]).toMatchObject({ line: 1, column: 1 }); // manifest
